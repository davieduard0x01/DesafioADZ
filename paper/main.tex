...
entendimento durante o estudo e quais trade-offs foram aceitos de propósito. As Seções~4
e~5 tratam do que foi efetivamente construído e de como executá-lo. A Seção~6 nomeia onde a
...
gestor; não existe caminho de \code{reason} direto para \code{act}. Omitidas para
...
quando as linhas passam de algumas dezenas. É o custo direto de recusar o sandbox, e é o
assunto da Seção~6.1.
...
% ---------------------------------------------------------------------------
% TODO-PASSADA-2 =============================================================
% Esta seção só pode ser escrita depois que o protótipo estiver rodando.
% Conteúdo previsto: (a) o que o avaliador vê na demo, sem uma única afirmação
% que não se confirme abrindo a URL; (b) um turno completo descrito passo a
% passo, nó por nó, com as tools realmente chamadas. Responde P8 do roteiro.
% NÃO ESCREVER NADA ESPECULATIVO AQUI.
% ============================================================================
\section{Sistema}

\textit{[Seção preenchida na revisão final, a partir do protótipo executado. Nada é
afirmado aqui antes de ser verificável.]}

% TODO-PASSADA-2 =============================================================
% Conteúdo previsto: URL da demo; como o avaliador cola a OPENROUTER_API_KEY e
% onde ela é guardada; seletor de modelo; o que é mock e o que é real nos
% datasets; o que dá para testar. Responde P9 e P10 do roteiro.
% ============================================================================
\section{Notas de execução}

\textit{[Seção preenchida na revisão final: endereço da demo, chave de API, modelos e o
que exatamente é simulado.]}
...
Sem CodeAct, o cruzamento de gasto do Meta com leads do CRM por \code{utm\_content} é feito
pelo modelo lendo linhas. Com poucos criativos funciona. Com uma conta real de centenas de
anúncios, o modelo erra soma e agrupamento --- e erra \emph{em silêncio}, porque o número
errado é plausível. O agravante é específico deste caso: parte dos leads chega sem
\code{utm\_content} e simplesmente não aparece na tabela. A única coisa que impede o
artefato de mentir com cara de rigor é uma nota de rodapé do tipo ``12 leads chegaram sem
\code{utm\_content} e não entram nesta tabela''. Ou seja: a correção da peça mais importante
do relatório depende de o nó \code{respond} lembrar de preencher um campo opcional. Isso não
é uma garantia; é uma esperança.
...
\paragraph{Agregação determinística dentro das tools.} Mover o join por \code{utm\_content},
o agrupamento e o cálculo de custo por lead para dentro de uma tool que faz aritmética em
código, devolvendo ao modelo o resultado agregado com a contagem explícita do que ficou de
fora. Isso mata a falha da Seção~6.1 sem abrir sandbox: o código é meu, não gerado. O custo
é rigidez --- cada agregação nova é uma tool nova --- e eu aceito, porque errar soma em
silêncio é pior que não conseguir agregar de um jeito específico.
