...
\usepackage{lmodern,booktabs,graphicx,microtype,geometry,array}
...
\author{Davi Eduardo\\[2pt]
{\normalsize Candidato $\cdot$ Desafio Harness Agêntico $\cdot$ AdzHub Núcleo Fundacional}}
\date{Agosto de 2026}
...
\begin{abstract}
\noindent
O gestor de conta não faz pedidos de um passo: ``por que caíram as vendas da Ômega 3 essa
semana?'' exige resolver uma entidade na memória da conta, consultar três APIs, cruzar
identificadores, descartar hipóteses e, no fim, defender o número na reunião com o cliente.
Proponho \textbf{Lastro}, um harness híbrido para o chat agêntico da AdzHub: \textbf{grafo
de estados por fora}, \textbf{loops ReAct dentro dos nós de investigação} e
\textbf{permissões \emph{deny-first} atravessando tudo}, com o supercérebro --- grafo de
entidades mais linha do tempo --- acessado por tool em vez de despejado no prompt. O
enquadramento é o argumento central: os cinco tipos de harness de referência não estão no
mesmo nível de abstração --- loop, topologia, ambiente de execução, camada transversal e
estratégia de acesso a contexto ---, então a pergunta não é ``qual escolher'', e sim o que
fica por fora, o que fica por dentro e o que atravessa. A aposta de produto: em marketing, a
\emph{defensibilidade} da resposta vale mais que a autonomia do agente. Dois recortes são
declarados com seu custo: sem sandbox/CodeAct e sem RLM completo. O protótipo roda os quatro
percursos do gestor e mostra o gate interrompendo o turno de verdade --- aprovar executa a
ação uma vez, reenviar a mesma aprovação executa zero ---, mas nenhum efeito sai dele: as
sete fontes de dados são arquivos locais. O paper nomeia onde a arquitetura quebra nas
quatro tarefas reais do gestor, em vez de alegar cobertura.
\end{abstract}

\smallskip
\noindent\textbf{Keywords:} harness agêntico; grafo de estados; ReAct;
...
\paragraph{Mapa.} A Seção~2 (\emph{Preliminaries}) fixa o vocabulário e lê o domínio. A
Seção~3 (\emph{Design}) é o coração argumentativo: por que grafo mais ReAct mais permissão,
por que não cada uma das outras referências isoladamente, o que é tool, o que é memória, o
que é App, o que mudou no meu entendimento durante o estudo e quais trade-offs foram aceitos
de propósito. A Seção~4 (\emph{System}) descreve o harness que foi construído e está no ar,
com um turno percorrido nó por nó, e diz peça por peça o que é tese, o que é simulado e o
que está visível na interface (Tabela~\ref{tab:pecas}). A Seção~5 (\emph{Execution notes})
diz como executá-lo --- chave, modelo, dados e o que os testes cobrem. A Seção~6
(\emph{Limitations}) nomeia onde a arquitetura quebra nas quatro tarefas reais do gestor, o
que eu construiria em seguida e o que deliberadamente não construiria.
...
\centering
\scriptsize
\caption{Cada peça do harness, o papel que cumpre na tese e o seu \emph{status de verdade}:
\textbf{só tese} (argumentada aqui, não existe em código), \textbf{simulado} (roda, mas
contra os sete JSON locais de \code{data/}) ou \textbf{visível na UI} (o avaliador enxerga
acontecer no trace ou no Palco). Nenhuma escrita atravessa a fronteira do protótipo. A
tabela é \textbf{ilustrativa} do contrato de fronteira --- não é allowlist de produção, que
seria por conta, por perfil e versionada.}
\label{tab:pecas}
\setlength{\tabcolsep}{5pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{32mm}>{\raggedright\arraybackslash}p{70mm}>{\raggedright\arraybackslash}p{61mm}@{}}
\toprule
\textbf{Peça} & \textbf{Papel na tese} & \textbf{No paper / no chat / mock} \\
\midrule
\code{graph\_query}          & memória: navega o grafo do supercérebro; resolve entidade em \code{interpret}, \code{fetch}, \code{reason} & simulado (\code{supercerebro.json}) $\cdot$ visível na UI \\
\code{timeline\_query}       & memória: eventos datados por entidade e janela & simulado (\code{timeline.json}) $\cdot$ visível na UI \\
\code{meta\_ads\_insights}   & dado bruto de mídia por anúncio; nó \code{fetch} & simulado (\code{meta\_ads.json}) $\cdot$ visível na UI \\
\code{google\_ads\_insights} & dado bruto do segundo canal; nó \code{fetch} & simulado (\code{google\_ads.json}) $\cdot$ visível na UI \\
\code{ga\_report}            & sessões por canal e landing; corrobora a quebra de atribuição & simulado (\code{ga.json}) $\cdot$ visível na UI \\
\code{crm\_leads}            & resultado real; é o outro lado do join por \code{utm\_content} & simulado (\code{crm.json}) $\cdot$ visível na UI \\
\code{list\_criativos}       & App: ranking e recomendação por criativo & simulado (\code{criativos.json}) $\cdot$ visível na UI \\
\code{get\_metrics}          & join Meta $\times$ CRM feito em código, com contagem do que ficou de fora & simulado (dois JSON) $\cdot$ visível na UI \\
\code{app\_diagnostico}      & App: metodologia que devolve estrutura --- veredito, causas com evidência, hipóteses descartadas & simulado $\cdot$ visível na UI \\
\code{propose\_ctas}         & App: gera copy; é \emph{leitura}, porque gerar não é publicar & simulado $\cdot$ visível na UI \\
\code{pause\_ads}            & escrita; só existe no nó \code{act}, depois do gate & simulado $\cdot$ visível na UI $\cdot$ \textbf{o efeito não sai do protótipo} \\
\code{send\_whatsapp}        & escrita; só existe no nó \code{act}, depois do gate & simulado $\cdot$ \textbf{o efeito não sai do protótipo} $\cdot$ só alcançável \emph{com} chave: nenhum dos quatro percursos gravados a propõe \\
\midrule
nó \code{gate} (não é tool)  & permissão como \emph{fase do fluxo}: interrompe o turno e devolve o preview antes de qualquer escrita & visível na UI --- o turno fecha em \code{awaiting\_confirmation} e a retomada é idempotente \\
supercérebro (grafo + linha do tempo) & memória de primeira classe, acessada por tool e nunca colada no prompt & simulado (39 nós, 76 arestas, 18 eventos) $\cdot$ visível na UI pelo trace \\
Palco (artefatos)            & preview e evidência antes da decisão, não depois dela & visível na UI (tabela, lista de criativos, \emph{diff} de CTA, pauta) \\
\midrule
\code{app\_mapa\_solucao} e \code{search\_conversations} & ficha de marca e memória de canal previstas no contrato oficial & \textbf{só tese} --- não existem como arquivo próprio (Seção~5.3) \\
checkpoint persistente entre sessões & retomar um turno depois de reiniciar o servidor & \textbf{só tese} --- o estado vive no turno \\
retry com backoff exponencial & resiliência de tool sob falha transitória & \textbf{só tese} \\
integração com API real (Meta, CRM, WhatsApp) & o efeito chegar na conta do cliente & \textbf{só tese} \\
sandbox/CodeAct e RLM completo & recortes recusados por escrito, com o custo nomeado & \textbf{só tese} (Seções~3.1 e~6.6) \\
\bottomrule
\end{tabular}
\end{table*}
...
\textbf{Tool}, por fim, é o contrato. A Tabela~\ref{tab:pecas} lista o catálogo desenhado:
...
consegue fazer?'' cabe na Tabela~\ref{tab:pecas}.
...
Os nove nós da Figura~\ref{fig:grafo} existem no runtime, com os tetos que a Seção~3.3
...
\code{act} tem teto de dois. O catálogo é o da Tabela~\ref{tab:pecas}: doze tools, dez de
...
verificável no trace: um único evento \code{action\_executed} por decisão, e
\code{executedActions} vazio quando o gestor nega. Reenviar a mesma decisão pela API é
recusado com \emph{``esta decisão já foi aplicada neste turno''}, sem executar nada --- e a
interface sequer oferece o segundo clique, porque o painel fecha ao decidir. A garantia vale
dentro da instância que atendeu o turno; o limite de infraestrutura está na Seção~5.5.
...
quantitativa (item 1 da Seção~6.5) e qualquer integração real ficaram de fora --- as sete
...
São 462\,kB de JSON em sete arquivos: o supercérebro com 39 nós e 76 arestas, a linha do
...
\paragraph{O contrato oficial, conferido depois.} O dataset foi gerado antes de eu ter o
\code{dataset\_prompt.md} em mãos --- o esquema saiu do próprio enunciado --- e só depois foi
conferido contra o contrato oficial. Registro as divergências em vez de as apagar, porque
elas dizem o que um avaliador vai procurar e não vai achar com o nome que espera.

{\raggedright
\textbf{Nomenclatura.} Os arquivos aqui são \code{supercerebro.json},
\code{timeline.json}, \code{meta\_ads.json}, \code{crm.json}, \code{criativos.json},
\code{ga.json} e \code{google\_ads.json}; o contrato sugere
\code{supercerebro\_graph.json}, \code{supercerebro\_timeline.json},
\code{api\_meta\_ads.json}, \code{api\_crm\_leads.json},
\code{app\_analise\_criativos.json}, \code{app\_mapa\_solucao.json} e
\code{conversas.json}. Os nomes de tool divergem na mesma medida ---
\code{graph\_query} onde o contrato diz \code{search\_client\_context},
\code{timeline\_query} por \code{get\_timeline}, \code{crm\_leads} por \code{get\_leads},
e assim por diante. Nenhuma dessas diferenças é semântica: a malha que o contrato exige ---
\code{ad\_id} do Meta igual a \code{utm\_content} do CRM, pessoas do grafo aparecendo na
linha do tempo, criativos do App batendo com os anúncios --- está fechada e é o que os
testes verificam.\par}

\textbf{Duas lacunas reais.} O contrato pede um \code{app\_mapa\_solucao} --- a ficha de
marca: oferta, promessa, prova, objeções, tom de voz --- e um \code{conversas.json}
separado, para reunião e WhatsApp. \textbf{Nós não temos os dois como arquivos próprios.} O
conteúdo de conversas vive dentro de \code{timeline.json}, o que basta para a pauta de T3 e
não basta para uma busca textual sobre o que foi dito; a ficha de marca simplesmente não
existe, e é ela que deveria ancorar o tom das variações de CTA em T4. As duas aparecem como
\emph{só tese} na Tabela~\ref{tab:pecas}.

\textbf{Uma regra do contrato que foi cumprida.} A regra 2 --- ``não escreva a resposta no
campo \code{notes}, deixe o harness descobrir'' --- estava violada: havia em \code{ga.json}
uma nota que descrevia a causa-raiz da quebra de atribuição em uma frase. Ela foi removida.
Nenhuma tool devolvia esse campo ao modelo, o que foi verificado no stream, então a
descoberta da Seção~4.2 nunca dependeu dela --- mas um dado que carrega o gabarito não tem
por que existir.
...
Há um segundo limite, e ele é do \emph{deploy}, não do desenho. O checkpoint que sustenta a
retomada do gate vive na memória do processo. A demo roda em ambiente \emph{serverless}, onde
a requisição que traz a decisão do gestor raramente cai na mesma instância que montou a
proposta --- e a retomada não encontrava o turno. No caminho de replay isso foi resolvido sem
persistência: como o turno é determinístico, o servidor o reexecuta em silêncio para
reconstruir o checkpoint e então retoma. \textbf{O caminho com chave não tem essa saída},
porque reexecutar chamaria o modelo de novo: ali a retomada depende de a decisão cair na
instância certa, e quando não cai o turno responde que não encontrou proposta pendente, em vez
de agir por conta. A correção de produção é mover o checkpoint para um \emph{store} externo,
compartilhado entre instâncias. O que interessa para a tese é que a falha é de infraestrutura,
não de desenho, e que o modo degradado é seguro: quando o checkpoint some, o turno recusa e
pede o pedido de novo --- nenhuma ação é executada por engano.

% ---------------------------------------------------------------------------
\section{Limitations}

As quatro primeiras subseções nomeiam uma falha por tarefa típica do gestor --- relatório,
diagnóstico, pauta, criativos. São falhas do harness que eu propus, não do estado da arte, e
nenhuma delas tem correção prevista aqui. As duas últimas dizem o que eu faria com mais uma
semana e o que eu deliberadamente deixaria de fazer.
...
\subsection{Se eu tivesse mais uma semana}
